...
const PanelOperations = ({ icon, text, onOpen, setIsActive, isActive, iconColor }) => {
    ...
    return (
        <div className='flex w-full gap-2 items-center border p-2 rounded-full cursor-pointer medium-phone:justify-evenly' onClick={() => { onOpen(); setIsActive(isActive) }}>
            <div className={`${iconColor}  small-phone:h-[1rem] small-phone:w-[1rem] medium-phone:h-[1.5rem] medium-phone:w-[1.5rem] tablet:h-[2rem] tablet:w-[2rem]  rounded-full`}>
                <p>{icon}</p>           
            </div>
